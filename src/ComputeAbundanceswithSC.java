 ...
 *
 * 2014.12.03 Milan:
 *  some input files in v4 have spectra intensities which are huge, need to
 *  switch from ints to longs.
 */
...
public class ComputeAbundanceswithSC {

    /**
     * @param args
     */
    private static Map<String, Map<String, Double>> protein2peptideCount = new HashMap<String, Map<String, Double>>();
    private static Map<String, Map<String, Long>> protein2peptideRawCount = new HashMap<>();
    ...
    private static Map<String, Long> peptideCount = new HashMap<String, Long>();
    private static Map<Integer, Integer> allPepLengths = new HashMap<Integer, Integer>();
    private static Map<Integer, Long> observedPepLengths = new HashMap<>();
    ...
    private static Map<String, Integer> Spectralcount = new HashMap<String, Integer>();
    ...
    public static void main(String[] args) throws IOException {
        ...
        int minPepLength = readPeptides(pepFile);
        while (minPepLength != 0) {
            if (minPepLength < 4) {
                System.err.println("error: peptide of length " + minPepLength + " in your peptide list");
                System.err.println("       peptides shorter than 4 AAs are not allowed for efficiency reasons");
                System.exit(1);
            }
            prefixSize = minPepLength;
            minPepLength = readPeptides(pepFile);
        }
        ;
        ...
        for (String p : proteinAbundance.keySet()) {
            if (normalize)
                System.out.println(p + "\t" + proteinAbundance.get(p) * 1e6 / total + "\t" + Spectralcount.get(p));
            else
...
//		System.err.println("peptide count: " + peptide_count);
    ...
    private static int rawspectralcount(String protein) {
        if (!protein2peptideRawCount.containsKey(protein))
            return -1;
        int peptide_Rawcount = 0;
        int peptide_coverage = 0;
        for (String pep : protein2peptideRawCount.get(protein).keySet()) {
            peptide_Rawcount += protein2peptideRawCount.get(protein).get(pep);
            //System.err.println("* " + pep + "\t" + peptideCount.get(protein).get(pep));
        }
        if (peptide_Rawcount == 0 || !sequences.containsKey(protein))
            return -1;
        //double adjusted_length = 0.0;
        //for (String pep : sequences.get(protein).split("R|K")) {
        //int len = pep.length() + 1;
    ...
    private static int readPeptides(final String filename) throws FileNotFoundException, IOException {
        BufferedReader reader = new BufferedReader(new FileReader(filename));
        String line;
        int line_count = 0;
        while ((line = reader.readLine()) != null) {
            ...
            String lookup = "";
            if (pepseq.length() < prefixSize) {
                System.err.println("warning: lookup prefix longer than shortest peptide - resetting now to smaller value and trying again...");
    ...
    private static void mapPeptides() {
                ...
                if (peptideLookup.containsKey(sub)) {    // are there any matching prefixes?
                    ...
                    for (final String peptide : peps.toArray(new String[peps.size()])) {
                        if (seq.regionMatches(i, peptide, 0, peptide.length())) {
                            ...
                            //							System.out.println(peptide + " matches " + id);
                        }
                    }
                }
            }
        ...
        for (final String pep : peptide2protein.keySet()) {
            for (final String protein : peptide2protein.get(pep)) {
                if (!protein2peptideRawCount.containsKey(protein))
                    protein2peptideRawCount.put(protein, new HashMap<String, Long>());
                protein2peptideRawCount.get(protein).put(pep, peptideCount.get(pep));
            }
        }
        for (final String pep : peptide2protein.keySet()) {
            for (final String protein : peptide2protein.get(pep)) {
