...
datasetNames <- c(commandArgs(trailingOnly = T)[4], commandArgs(trailingOnly = T)[5], commandArgs(trailingOnly = T)[6], commandArgs(trailingOnly = T)[7], commandArgs(trailingOnly = T)[8], commandArgs(trailingOnly = T)[9])


# REPLACE weights, use same order as datasetNames() #e.g. 100% == 1; 50% == 0.5; 1% == 0.01
weights <- c(as.numeric(commandArgs(trailingOnly = T)[10]), as.numeric(commandArgs(trailingOnly = T)[11]), as.numeric(commandArgs(trailingOnly = T)[12]), as.numeric(commandArgs(trailingOnly = T)[13]), as.numeric(commandArgs(trailingOnly = T)[14]), as.numeric(commandArgs(trailingOnly = T)[15]))
...
write.table(mergeFile, file=paste(pathOutput, "mergedFile_weight_", as.numeric(commandArgs(trailingOnly = T)[10]), "_", as.numeric(commandArgs(trailingOnly = T)[11]), "_", as.numeric(commandArgs(trailingOnly = T)[12]), "_", as.numeric(commandArgs(trailingOnly = T)[13]), "_", as.numeric(commandArgs(trailingOnly = T)[14]), "_", as.numeric(commandArgs(trailingOnly = T)[15]),"_all.txt", sep=""), quote=F, sep="\t", row.names=F)
write.table(mergeFile[,c(1,(length(datasetNames)+2))], file=paste(pathOutput,"integrData_weighted_", as.numeric(commandArgs(trailingOnly = T)[10]), "_", as.numeric(commandArgs(trailingOnly = T)[11]), "_", as.numeric(commandArgs(trailingOnly = T)[12]), "_", as.numeric(commandArgs(trailingOnly = T)[13]), "_", as.numeric(commandArgs(trailingOnly = T)[14]), "_", as.numeric(commandArgs(trailingOnly = T)[15]),".txt", sep=""), quote=F, sep="\t", row.names=F, col.names=F)

#print(mergeFile[1:5,])
print("################# NEW FILE ########################", quote=F)
print("", quote=F)
print("weights of files: ", quote=F)
print(paste(as.numeric(commandArgs(trailingOnly = T)[10]), commandArgs(trailingOnly = T)[4]), quote=F)
print(paste(as.numeric(commandArgs(trailingOnly = T)[11]), commandArgs(trailingOnly = T)[5]), quote=F)
print(paste(as.numeric(commandArgs(trailingOnly = T)[12]), commandArgs(trailingOnly = T)[6]), quote=F)
print(paste(as.numeric(commandArgs(trailingOnly = T)[13]), commandArgs(trailingOnly = T)[7]), quote=F)
print(paste(as.numeric(commandArgs(trailingOnly = T)[14]), commandArgs(trailingOnly = T)[8]), quote=F)
print(paste(as.numeric(commandArgs(trailingOnly = T)[15]), commandArgs(trailingOnly = T)[9]), quote=F)
print("", quote=F)
