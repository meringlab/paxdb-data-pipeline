...
datasetNames <- c(commandArgs(trailingOnly = T)[4], commandArgs(trailingOnly = T)[5], commandArgs(trailingOnly = T)[6])


# REPLACE weights, use same order as datasetNames() #e.g. 100% == 1; 50% == 0.5; 1% == 0.01
weights <- c(as.numeric(commandArgs(trailingOnly = T)[7]), as.numeric(commandArgs(trailingOnly = T)[8]), as.numeric(commandArgs(trailingOnly = T)[9]))
...
write.table(mergeFile, file=paste(pathOutput, "mergedFile_weight_", as.numeric(commandArgs(trailingOnly = T)[7]), "_", as.numeric(commandArgs(trailingOnly = T)[8]), "_", as.numeric(commandArgs(trailingOnly = T)[9]),"_all.txt", sep=""), quote=F, sep="\t", row.names=F)
write.table(mergeFile[,c(1,(length(datasetNames)+2))], file=paste(pathOutput,"integrData_weighted_", as.numeric(commandArgs(trailingOnly = T)[7]), "_", as.numeric(commandArgs(trailingOnly = T)[8]), "_", as.numeric(commandArgs(trailingOnly = T)[9]),".txt", sep=""), quote=F, sep="\t", row.names=F, col.names=F)

#print(mergeFile[1:5,])
print("################# NEW FILE ########################", quote=F)
print("", quote=F)
print("weights of files: ", quote=F)
print(paste(as.numeric(commandArgs(trailingOnly = T)[7]), commandArgs(trailingOnly = T)[4]), quote=F)
print(paste(as.numeric(commandArgs(trailingOnly = T)[8]), commandArgs(trailingOnly = T)[5]), quote=F)
print(paste(as.numeric(commandArgs(trailingOnly = T)[9]), commandArgs(trailingOnly = T)[6]), quote=F)
print("", quote=F)
