...
#  2) output folder
#  3) mRNA file
#  4-.. dataset names (without the '.txt' extension) 
#  last two) weights
...
datasetNames <- c(commandArgs(trailingOnly = T)[4], commandArgs(trailingOnly = T)[5])


# REPLACE weights, use same order as datasetNames() #e.g. 100% == 1; 50% == 0.5; 1% == 0.01
weights <- c(as.numeric(commandArgs(trailingOnly = T)[6]), as.numeric(commandArgs(trailingOnly = T)[7]))
...
write.table(mergeFile, file=paste(pathOutput, "mergedFile_weight_", as.numeric(commandArgs(trailingOnly = T)[6]), "_", as.numeric(commandArgs(trailingOnly = T)[7]),"_all.txt", sep=""), quote=F, sep="\t", row.names=F)
write.table(mergeFile[,c(1,(length(datasetNames)+2))], file=paste(pathOutput,"integrData_weighted_", as.numeric(commandArgs(trailingOnly = T)[6]), "_", as.numeric(commandArgs(trailingOnly = T)[7]),".txt", sep=""), quote=F, sep="\t", row.names=F, col.names=F)
